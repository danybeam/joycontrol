...
# Press A for prompt
    await button_push(ControllerState, 'a')
...
# Dowm 1 for rotto lotto
    await button_push(ControllerState, 'down')
...
# A save
    await button_push(ControllerState, 'a')
    print('Trigger save')
    await asyncio.sleep(3)  # wait 5 segundos
# A(end text progress)
    await button_push(ControllerState, 'a')
...
# A(exit texxt)
    ...
    await asyncio.sleep(2)
# CHANGE DAY
    await switchDayAndReturn(ControllerState, connected=True, first=first)
